...
# Create a for loop that iterates through the elements of an array an prints out each value

begin
	Example_Array = rand(20) # a vector (single-row) of 20 random numbers
	for x in Example_Array
		println(x)
	end
end
...
# ╔═╡ 8f600670-d2fb-4fb0-a963-e8fd2f479485
# Do another example with a Tuple or array (in class)

# ╔═╡ cb7e798e-d235-4c21-9365-9a79cbc1a5f4
begin
	n = 1  # We have to tell Julia what the value of x is in order for the conditional statement to be understood...
	while n < 50  # if we didn't specify x =1 above, it could have any value here.
		println(sum(1:n))
...
# Run this cell multipple times for it to be understood.
...
# ╔═╡ fa5ced75-dd4b-41b1-a830-9850c31cb393
println(m)
...
# Remember, the keys of a dictionary are unique
...
begin
	...
	"Paul" in keys(Phonebook) ? nothing : println("I can't find that person in the phonebook... 😕")
